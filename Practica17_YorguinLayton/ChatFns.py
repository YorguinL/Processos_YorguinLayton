...
def LoadOtherEntry(ChatLog, NomUsuari, EntryText):
    if EntryText != '':
        ChatLog.config(state=NORMAL)
        if ChatLog.index('end') != None:
            try:
                ...
                pass
            ChatLog.insert(END, NomUsuari + ": "+ EntryText)
            ChatLog.tag_add(NomUsuari, LineNumber, LineNumber+float("0."+str(len(NomUsuari)+1)))
            ChatLog.tag_config(NomUsuari, foreground="#04B404", font=("Arial", 12, "bold"))
            ChatLog.config(state=DISABLED)
            ChatLog.yview(END)

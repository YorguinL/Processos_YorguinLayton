...
def decipherMessage(encryptedMsg, key):
    cipher_rsa = PKCS1_OAEP.new(key)
    dec_msg = cipher_rsa.decrypt(encryptedMsg).decode("utf-8")
    return dec_msg
...
def encryptAES(msg, key):
    ...
    enc_msg = cipher_aes.encrypt(p_msg.encode())
...
def decipherAES(msg, key):
    ...
    cipher_aes = AES.new(key, AES.MODE_CBC, iv)
    dec_msg = cipher_aes.decrypt(enc_msg).decode("utf-8")
...
if __name__ == '__main__':
    ...
    enc_data = encryptMessage("BONES COVID", pbc_key)
